...
class Display(View):
    ...
    def size(self, rect):
        return max(self.min_width, max(len(line) for line in self.lines)), len(self.lines)
...
class Box(View):
    ...
    def __init__(self, inner, caption=None, under_script=None, x_margin=1, y_margin=0, x_fill=False, y_fill=False):
        self.inner = inner
    ...
    def size(self, rect):
        ...
        if not self.x_fill or not self.y_fill:
            inner_size = self.inner.size(
                rect.adj_rect(1 + self.x_margin, 1 + self.y_margin, 1 + self.x_margin, 1 + self.y_margin))
            if not self.x_fill:
                w = inner_size[0] + 2 * (1 + self.x_margin)
            if not self.y_fill:
                h = inner_size[1] + 2 * (1 + self.y_margin)
        ...
        if rect_w > 0 and rect_h > 0:
            ...
            try:
                ...
                if self.under_script:
                    s = self.under_script.size(rect)
                    self.under_script.display(rect.adj_rect(max(3, rect_w - s[0] - 3), rect_h - 1))
            except curses.error as e:
...
class Button(Control):
    ...
    def on_event(self, ev):
        if ev.type == 'key' and (is_enter(ev) or ev.what == ord(' ') and self.on_click):
...
class PreviewPane(Control):
    ...
    def render(self, app):
        ...
        display_lines = list(self._text[l_start + self.h_scroll_offset:l_end] for l_start, l_end in
                             self._lines[self.v_scroll_offset:self.v_scroll_offset + MAX_HEIGHT])
        if self.row_selectable and focused:
            hi_offset = self.selected_row - self.v_scroll_offset
            self.last_render = Vertical([
                Display(line, attr=attr + (curses.A_STANDOUT if i == hi_offset else 0)) for i, line in
                enumerate(display_lines)
            ])
        else:
    ...
    def on_event(self, ev):
        if ev.type == 'key':
            ...
            if self.row_selectable:
                # We scroll the focus
                change, self.selected_row, self.v_scroll_offset = handle_scroll_key(ev.key, self.selected_row,
                                                                                    len(self._lines),
                                                                                    self.v_scroll_offset,
                                                                                    self.last_render.rect.h,
                                                                                    page_size=30)
            else:
                # We scroll the screen
                change, self.v_scroll_offset, _ = handle_scroll_key(ev.key, self.v_scroll_offset, len(self._lines),
                                                                    self.v_scroll_offset, self.last_render.rect.h,
                                                                    page_size=30)
            ...
            if ev.key == ord('s'):
                edit_popup(ev.app, self._save_contents, value='report.log', caption='Save to file')
            if is_enter(ev) and self.row_selectable and self.on_select_row and 0 <= self.row_selectable < len(
                    self._lines):
...
class Rect(object):
    def __init__(self, app, screen, x, y, w, h):
        self.app = app
        self.screen = screen  # type: curses.window
        self.x = x
        self.y = y
        self.w = w
        self.h = h
    ...
    def __repr__(self):
        return '<class %s size=(%s,%s,%s,%s)>' % (type(self).__name__, self.x, self.y, self.w, self.h)
...
class Event(object):
    def __init__(self, type_, what, target, app):
        self.type = type_
        self.key = what
        self.what = what
        self.target = target
...
class App(Control):
    ...
    def find_ancestor(self, ctrl, set_):
